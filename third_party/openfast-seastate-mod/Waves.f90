...
!JASON: MOVE THIS USER-DEFINED ROUTINE (UserWaveSpctrm) TO THE UserSubs.f90 OF HydroDyn WHEN THE PLATFORM LOADING FUNCTIONALITY HAS BEEN DOCUMENTED!!!!!
	  ...
	  !=======================================================================
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE StillWaterWaves_Init ( InitInp, InitOut, WaveField, ErrStat, ErrMsg )
...
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE VariousWaves_Init ( InitInp, InitOut, WaveField, FileName, ErrStat, ErrMsg )
...
!----------------------------------------------------------------------------------------------------------------------------------
   ...
   INTEGER                      :: ucount
   LOGICAL                      :: found  
   INTEGER(IntKi) 				:: dirIdx
   REAL(SiKi), ALLOCATABLE 		:: weights(:), normalizedWeight(:) ! Weights from spreading function
   REAL(SiKi)                   :: WvSpreadCos2SConst       ! Normalization constant for wave spreading function.
   REAL(SiKi)					:: totalWeight
   COMPLEX(SiKi), ALLOCATABLE   :: tmpFilt(:)   
   INTEGER 						:: unitOrig					! unit for file with original wave
   INTEGER(IntKi)               :: unitDir                  ! unit for file with directional components
   INTEGER(IntKi)               :: unitSpec                 ! unit for file Comp
   CHARACTER(len=:), ALLOCATABLE :: base
   CHARACTER(len=256)           :: fname					! Base name for the files
   CHARACTER(len=500) 			:: headerLine				! Header line for files
   ...
   ALLOCATE ( weights                 (1:InitOut%WaveNDir),                          STAT=ErrStatTmp)
   ALLOCATE ( normalizedWeight                 (1:InitOut%WaveNDir),                          STAT=ErrStatTmp)
   ALLOCATE ( WaveField%WaveElevDir   (1:InitOut%WaveNDir, 0:WaveField%NStepWave-1), STAT=ErrStatTmp); if (Failed0('WaveElevDir   ')) return;
...
!! ==========================================================
   ...
   IF (WaveField%WaveMod == WaveMod_UserFreq) THEN
	  ...
	  DO K = 1, WaveField%NStepWave2
	     found = .FALSE.
	     ! the direction you just read:
		 DO J = 1, ucount
		    IF (ABS( WvTheta(j) - WaveField%WaveDirArr(k) ) < 1e-6 ) THEN
			   found = .TRUE.
			   EXIT
            END IF
         END DO
         IF (.NOT. found) THEN
            ucount = ucount + 1
			WvTheta(ucount) = WaveField%WaveDirArr(k)
         END IF
      END DO
   ELSE
      CALL CalculateWaveSpreading(InitInp, InitOut, WaveField, WvTheta, WvSpreadCos2SConst, ErrStatTmp, ErrMsgTmp)
	  CALL SetErrStat(ErrStatTmp, ErrMsgTmp, ErrStat, ErrMsg, 'VariousWaves_Init')
	  IF (ErrStat >= AbortErrLev) RETURN
   END IF
	
   IF (.NOT. ALLOCATED(tmpFilt)) THEN
      IF (WaveField%WaveMod == WaveMod_UserFreq) THEN
         ALLOCATE( tmpFilt( 0:WaveField%NStepWave2) )
	  ELSE
         ALLOCATE( tmpFilt( 0:WaveField%NStepWave - 1 ) )
	  END IF
   END IF
		 
   ! Initialize WaveElevDir to zero, just in case:
   WaveField%WaveElevDir = 0.0_SiKi
   totalWeight = 0.0_SiKi
		 
   ! Compute each bin’s width
   DO d = 1, InitOut%WaveNDir
      IF (d == 1) THEN
         dtheta(d) = WvTheta(2) - WvTheta(1)
      ELSE IF (d == InitOut%WaveNDir) THEN
         dtheta(d) = WvTheta(d) - WvTheta(d-1)
      ELSE
         dtheta(d) = 0.5_SiKi * (WvTheta(d+1) - WvTheta(d-1))
      END IF
   END DO
   
   ! Generate COMP file when spreading is created by OpenFAST
   SELECT CASE ( WaveField%WaveMod )
      CASE(WaveMod_Regular, WaveMod_JONSWAP, WaveMod_WhiteNoise, WaveMod_UserSpctrm)
		 ! Computing weights if we have spreading
		 DO d = 1, InitOut%WaveNDir
		    IF ( InitInp%WaveDirSpread > 0 ) THEN
			   weights(d) = (WvSpreadCos2SConst*abs( cos(Pi*(WvTheta(d)-WaveField%WaveDir)/InitInp%WaveDirRange) ) **(2*InitInp%WaveDirSpread)) * (dtheta(d) * Pi/180.0_SiKi)
	           totalWeight = totalWeight + weights(d)
			END IF
	     END DO
			   
		 ! If totalWeight is zero (should not occur in typical situations) assign equal weights:
	     IF (totalWeight == 0.0_SiKi) THEN
            DO d = 1, InitOut%WaveNDir
               weights(d) = 1.0_SiKi / real(InitOut%WaveNDir, SiKi)
            END DO
            totalWeight = 1.0_SiKi
         ENDIF
		  
	     ! Compute normalized weights:
	     DO d = 1, InitOut%WaveNDir
            normalizedWeight(d) = weights(d) / totalWeight 
         END DO
		 
	     ! Write the COMP file when the spectrum is generated by OpenFAST
	     OPEN( NEWUNIT=unitSpec, FILE='DiscreteSpectrum.Comp', STATUS='replace', ACTION='write', FORM='formatted', RECL=1000 )
	        WRITE(unitSpec,'(A)') 'omega [rad/s]   H [m]   beta [deg]   phi [deg]'
		    DO I = 1,WaveField%NStepWave2
		       dirIdx = 0
               DO d = 1, InitOut%WaveNDir
                  IF (WaveField%WaveDirArr(I) == WvTheta(d)) THEN
                     dirIdx = d      ! direction for this I
                     EXIT
                  END IF
               END DO 
			   IF (I == 1 .OR. I == WaveField%NStepWave2) THEN
                  factor = 1.0_SiKi
               ELSE
                  factor = 2.0_SiKi
               END IF
		       H      = ( factor * 2.0_SiKi * SQRT( WaveField%WaveElevC0(1,I)**2 +  WaveField%WaveElevC0(2,I)**2 ) &				
			              / REAL( WaveField%NStepWave, SiKi )	) * SQRT( InitOut%WaveNDir * normalizedWeight(dirIdx) )		   ! complex‐amp magnitude undo the IFFT length and account for single‐sided spectrum
		       phi    = ATAN2( WaveField%WaveElevC0(2,I), WaveField%WaveElevC0(1,I) ) * 180.0_SiKi/pi					       ! phase in degrees
			   WRITE(unitSpec,'(E17.10,2X,E12.6,2X,F7.2,4X,F7.2)') OmegaArr(I), H, WaveField%WaveDirArr(I), phi
			ENDDO
		 CLOSE(unitSpec)			     
   ENDSELECT
		 
   DO d = 1, InitOut%WaveNDir			
      ! Zero out tmpFilt every cycle:
      tmpFilt = CMPLX( 0.0_SiKi, 0.0_SiKi )

      ! Distribute the Fourier coefficient into each directional bin d:
	  IF (WaveField%WaveMod == WaveMod_UserFreq) THEN
		 ! Case if the spectra is defined by user
		 DO I = 0, WaveField%NStepWave2
		    IF ( WaveField%WaveDirArr(I) == WvTheta(d) ) THEN
			   tmpFilt(I) = tmpComplexArr(I)
			END IF
		 END DO
	  ELSE
	     ! Case for spreading definition
         DO I = 0, WaveField%NStepWave - 1
            IF ( WaveField%WaveDirArr(I) == WvTheta(d) ) THEN
               tmpFilt(I) = tmpComplexArr(I) * SQRT( InitOut%WaveNDir * normalizedWeight(d) )
			END IF
         END DO
      END IF

      ! Compute the inverse discrete Fourier transforms to find the time-domain directional waves
	  CALL ApplyFFT_cx( WaveField%WaveElevDir(d, 0:WaveField%NStepWave-1), tmpFilt, FFT_Data, ErrStatTmp )  
   END DO  ! over d = 1…WaveNDir
   ...
   !===========================================================
   ! Write the Directional Waves File
   !===========================================================
   OPEN(NEWUNIT=unitDir, FILE=TRIM("./Waves/DirectionalWaves_" // TRIM(base) // ".txt"), STATUS='replace', ACTION='write', FORM='formatted', RECL=1000)
      ! Header Section for the directional waves file:
	  WRITE(unitDir, '(A)') 'File containing the directional waves by Antonio Medina (antonio.medina.manuel@upm.es) 05-06-2025'
	  WRITE(unitDir, *)   ! Blank line
	  
	  WRITE(unitDir, '(A)') 'Angles (from WvTheta):'
	  WRITE(unitDir, *) WvTheta
	  WRITE(unitDir, '(A, I0, A)') 'Found quantiles of the energy from the spreading function for ', InitOut%WaveNDir, ' directions.'
	  WRITE(unitDir, *)   ! Blank line
	  
	  ! Write a header line for the data:
	  headerLine = 'Time (s)'
	  DO d = 1, InitOut%WaveNDir
	     WRITE(tempStr, '(A,I0)') '  WaveDir', d
	     headerLine = TRIM(headerLine) // TRIM(tempStr)
	  END DO
	  WRITE(unitDir, '(A)') headerLine
	  WRITE(unitDir, *)   ! Blank line
	  
	  ! Data Section for directional waves:
	  DO I = 0, WaveField%NStepWave
	     WRITE(unitDir, '(F10.4)', ADVANCE='no') WaveField%WaveTime(I)
		    DO d = 1, InitOut%WaveNDir
			   WRITE(unitDir, '(1X, F12.6)', ADVANCE='no') WaveField%WaveElevDir(d, I)
		    END DO
		 WRITE(unitDir, *)   ! End the line for this time step
	  END DO
	  
    CLOSE(unitDir)
	
	!===========================================================
	! Write the Original Waves File
	!===========================================================
	OPEN(NEWUNIT=unitOrig, FILE=TRIM("./Waves/OriginalWaves_" // TRIM(base) // ".txt"), STATUS='replace', ACTION='write', FORM='formatted')
	  
	! Header Section for the original waves file:
	WRITE(unitOrig, '(A)') 'File containing the original wave by Antonio Medina (antonio.medina.manuel@upm.es) 04-04-2025'
	WRITE(unitOrig, *)   ! Blank line
	WRITE(unitOrig, '(A)') 'Time (s)  Wave (m)'
	WRITE(unitOrig, *)   ! Blank line

	! Data section: Loop over each time step and write both numbers in one statement.
	DO I = 0, WaveField%NStepWave
		WRITE(unitOrig, '(F10.4,1X,F12.6)') WaveField%WaveTime(I), WaveField%WaveElev0(I)
	END DO
	  
	CLOSE(unitOrig)
	
	PRINT *, 'Files for waves written successfully.'
...
SUBROUTINE Waves_Init( InitInp, InitOut, WaveField, FileName, ErrStat, ErrMsg )
      ...
      CASE ( WaveMod_Regular, WaveMod_JONSWAP, WaveMod_WhiteNoise, WaveMod_UserSpctrm, WaveMod_RegularUsrPh )       ! 1, 10: Plane progressive (regular) wave, 2: JONSWAP/Pierson-Moskowitz spectrum (irregular) wave, 3: white-noise, or 4: user-defined spectrum (irregular) wave.
         ...
         CALL VariousWaves_Init( InitInp, InitOut, WaveField, FileName, ErrStatTmp, ErrMsgTmp )
         CALL SetErrStat(ErrStatTmp,ErrMsgTmp,ErrStat,ErrMsg,'Waves_Init')
         IF ( ErrStat >= AbortErrLev ) RETURN
...
!------------------------------------------------------------------------------------------------------------------------
SUBROUTINE CalculateWaveDirection(InitInp, InitOut, WaveField, ErrStat, ErrMsg )
      ...
      ELSE ! multi directional waves
         ...
         DO I = 1,WvSpreadFreqPerDir
            ...
            DO J = 1, InitOut%WaveNDir
               ...
               WaveField%WaveDirArr(K)   =  WvTheta( LastInd )
...
!------------------------------------------------------------------------------------------------------------------------
SUBROUTINE CalculateWaveSpreading(InitInp, InitOut, WaveField, WvTheta, WvSpreadCos2SConst, ErrStat, ErrMsg )
...
!----------------------------------------------------------------------------------------------------------------------------------
   ...
   ErrMsg  = ""
         ...
         DO I=0,WvSpreadNDir
			...
			! WvSpreadThetas(I) =  InitInp%WaveDirRange/(2.0_SiKi) + (I + 0.5_SiKi) * WvSpreadDTheta
...
SUBROUTINE Get_1Spsd_and_WaveElevC0(InitInp, InitOut, WaveField, OmegaArr, WaveS1SddArr)
