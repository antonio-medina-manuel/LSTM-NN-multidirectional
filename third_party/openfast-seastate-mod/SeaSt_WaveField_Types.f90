...
! =========  SeaSt_WaveFieldType  =======
  TYPE, PUBLIC :: SeaSt_WaveFieldType
    ...
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElevDir      !< New field ANTONIO: Directional Fourier components (e.g., [time, direction]) [-]
...
subroutine SeaSt_WaveField_CopySeaSt_WaveFieldType(SrcSeaSt_WaveFieldTypeData, DstSeaSt_WaveFieldTypeData, CtrlCode, ErrStat, ErrMsg)
   ...
   if (allocated(SrcSeaSt_WaveFieldTypeData%WaveElevDir)) then
      LB(1:2) = lbound(SrcSeaSt_WaveFieldTypeData%WaveElevDir)
      UB(1:2) = ubound(SrcSeaSt_WaveFieldTypeData%WaveElevDir)
      if (.not. allocated(DstSeaSt_WaveFieldTypeData%WaveElevDir)) then
         allocate(DstSeaSt_WaveFieldTypeData%WaveElevDir(LB(1):UB(1),LB(2):UB(2)), stat=ErrStat2)
         if (ErrStat2 /= 0) then
            call SetErrStat(ErrID_Fatal, 'Error allocating DstSeaSt_WaveFieldTypeData%WaveElevDir.', ErrStat, ErrMsg, RoutineName)
            return
         end if
      end if
      DstSeaSt_WaveFieldTypeData%WaveElevDir = SrcSeaSt_WaveFieldTypeData%WaveElevDir
   end if
...
subroutine SeaSt_WaveField_DestroySeaSt_WaveFieldType(SeaSt_WaveFieldTypeData, ErrStat, ErrMsg)
   ...
   if (allocated(SeaSt_WaveFieldTypeData%WaveElevDir)) then
      deallocate(SeaSt_WaveFieldTypeData%WaveElevDir)
   end if
...
subroutine SeaSt_WaveField_PackSeaSt_WaveFieldType(RF, Indata)
   ...
   call RegPackAlloc(RF, InData%WaveElevDir)
...
subroutine SeaSt_WaveField_UnPackSeaSt_WaveFieldType(RF, OutData)
   ...
   call RegUnpackAlloc(RF, OutData%WaveElevDir); if (RegCheckErr(RF, RoutineName)) return
