...
!----------------------------------------------------------------------------------------------------------------------------------
	  ...
	  ! ---ANTONIO---	  
	  ALLOCATE( UniqueDirs(WaveCompData%NCompWave), STAT=ErrStatTmp )
	  UniqueDirs = 0.0
	  ucount = 0.0
      ...
      DO I = 1, WaveCompData%NCompWave
         ...
         DO J = 1, ucount
            IF (ABS( UniqueDirs(j) - WaveField%WaveDirArr(i) ) < 1e-6 ) THEN
               found = .TRUE.
               EXIT
            END IF
         END DO
         IF (.NOT. found) THEN
            ucount = ucount + 1
            UniqueDirs(ucount) = WaveField%WaveDirArr(i)
         END IF
      END DO

      InitOut%WaveNDir = ucount
